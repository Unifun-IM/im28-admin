...
/** 查询群会话 按多个群 ID、单个群名称关键词、群主用户 ID 和群状态分页查询群会话。多个非空条件按 AND 组合；所有查询条件为空时固定返回空列表，不扫描全部群。需要 `conversations.groups.list` 权限。 POST /v1/admin/conversations/groups/list */
...
/** 查询指定用户的会话列表 按 user_id 分页查询用户当前及历史会话，只返回后台列表展示所需字段，需要 `conversations.list` 权限。 POST /v1/admin/conversations/list */
...
message.type 和 message.body 与 C 端消息协议一致：101=text、102=image、103=audio、104=video、105=file、108=card、110=custom、113=typing、114=quote、115=emoji、116=transfer、117=red_packet、1400=system。系统动作通过 body.system.event_type 区分。OpenAPI 的 MessageBody 会展开每种正文结构。
查询遵守该用户的历史清空、仅自己删除、消息过期/删除和群聊离群边界。data.users 是本页发送者资料去重列表，按 message.sender_id=data.users[].user_id 关联。需要 `conversations.messages.list` 权限。 POST /v1/admin/conversations/messages/list */
...
/** 查询用户会话入口列表 按用户 ID、昵称、手机号、邮箱或账号查询用户摘要，也支持批量提交最多 100 个 user_ids，需要 `conversations.users.list` 权限。页面首次进入且 keyword、user_ids 都为空时固定返回空列表，不会扫描全部用户。此接口不返回聊天内容；前端点击“查聊天”时使用返回的 user_id 进入后续查询。 POST /v1/admin/conversations/users/list */
