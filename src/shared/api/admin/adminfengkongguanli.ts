...
/** 添加 C 端 IP 黑名单 一次添加 1 至 100 个 IPv4 或 IPv6 地址；存在无效 IP 或已封控 IP 时整批失败。成功后 C 端 HTTP 请求会被拦截，后台接口不受影响。需要 `admin.risk.ip_blacklist.write` 权限和谷歌验证码。 POST /v1/admin/risk/ip-blacklist/add */
...
/** 批量恢复 IP 一次恢复 1 至 100 个当前有效的 IP 黑名单记录；任一 IP 不存在时整批失败。需要 `admin.risk.ip_blacklist.write` 权限和谷歌验证码。 POST /v1/admin/risk/ip-blacklist/batch-remove */
...
/** 查询 C 端 IP 黑名单 查询当前有效的 IP 封控记录，支持按 IP、原因和操作时间筛选。需要 `admin.risk.ip_blacklist.read` 权限。 POST /v1/admin/risk/ip-blacklist/list */
...
/** 恢复单个 IP 将一个当前有效的 IP 黑名单记录恢复为可访问状态。需要 `admin.risk.ip_blacklist.write` 权限和谷歌验证码。 POST /v1/admin/risk/ip-blacklist/remove */
