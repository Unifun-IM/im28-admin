...
- 同一实体的主值与 ID 已按 `admin-page` 决定合并时，有头像使用 `AvatarNameCell`，无头像使用 `DoubleLineCell`；用 `primaryLabel` / `secondaryLabel` 保留字段语义，需要复制对应行时传 `copyPrimary` / `copySecondary`。标签不进入复制值，页面不要手工拼两套文字与复制按钮。
