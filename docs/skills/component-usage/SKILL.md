...
5. 累计固定列、选择 / 展开列和至少一个弹性列的最小宽度，与侧栏展开后的常用内容区比较；只有真实放不下时才设置横向滚动。
...
| 普通名称、账号 | `120-184px` |
| 仅日期 | `112-128px` |
| 日期时间（精确到秒） | `176-184px`，必须验证完整值不省略 |
| 状态、类型、boolean | `88-112px` |
...
整表至少保留一个有意义的内容列作为弹性列，避免每列都固定后产生无效横向滚动。完成后用有代表性的最长数据在侧栏展开状态检查常用桌面内容区和窄屏：日期时间、表头、状态与操作不得截断，操作列也不得留出显著空白；不能只验证空表或短 mock 数据。
