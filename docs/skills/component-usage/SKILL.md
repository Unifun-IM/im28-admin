...
`ActionLinks variant="text"` 在桌面默认外露 1-2 项，超过 2 项时只外露第 1 项并提供更多菜单；`variant="icon"` 桌面默认最多 3 个 20px 槽位。`md` 及以下由公共组件压缩为一个 32px 入口：单操作直接使用语义图标，多操作全部进入更多菜单；公共表格同时把操作列归一为 72px。调用页不得通过 `window.innerWidth` 重复实现，也不要给移动操作列写局部宽度补丁。

动作数组按业务优先级排列：最高频、低风险动作在前，危险或低频动作靠后；公共组件保持该顺序，不擅自重排。隐藏在菜单中的操作不占桌面列宽。

桌面文字操作列宽度按“所有外露文案宽度 + 8px × 间隔数 + 更多按钮（存在时）+ 24px padding”计算，并与中英文表头宽度取较大值。动态启用 / 禁用、封禁 / 解封等按所有状态中最长文案计算，不额外添加经验性安全宽度。
