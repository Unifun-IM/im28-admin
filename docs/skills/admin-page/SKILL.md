...
生成 locale 和操作列前先精简操作文案。列表上下文已经明确对象时，操作项使用能独立表达动作的最短无歧义文案，不重复对象名称，也不保留没有信息增量的“查看 / 开启 / 关闭 / 执行”等前缀。例如：
...
会造成歧义、涉及多个对象或需要强调危险结果时保留必要宾语。按钮、确认弹窗和 locale 使用同一语义；先确定最终 zh-CN / en-US 文案，再计算操作列宽，禁止先用冗长占位文案定宽后只改显示文本。

生成 `columns` 前必须先做列宽分析，具体计算口径以 `docs/skills/component-usage/SKILL.md` 的“列宽分析”章节为准：

- Figma / PRD 明确列宽时优先采用；未明确时，根据最终中英文最长表头、格式化后的典型值、实际单元格形态、排序图标和单元格 padding 推导最小宽度。
- 先列出每列的内容类型和基础宽度，再写 `columns`；禁止按接口字段类型机械套同一个宽度，也禁止所有列都使用偏大的相同宽度。
- 当前 `BizListPage` 的 Arco 固定表格会按 `width` 比例铺满可用空间，且无 `width` 的可见列可能被压缩到接近 `0px`；因此每个可见数据列都要设置非零基础宽度。长文本或主要内容列通过更大的基础宽度占比吸收空间，禁止通过省略 `width` 制造弹性列。
- 操作列只计算 `ActionLinks` 实际外露的文字 / 图标、更多按钮、间距与操作列 padding；进入下拉菜单的操作不占列宽。动态操作文案按所有状态及 zh-CN / en-US 中最长者计算。
- 日期时间等稳定格式值在正常桌面宽度下应完整显示；发生省略时优先修正列宽，不能用 Tooltip 掩盖错误宽度。
- 列宽完成后累计所有数据列及选择 / 展开列的基础宽度，并在浏览器读取实际表头 / 单元格宽度；真实内容需要的最小宽度超过内容区时才接受横向滚动，不得用扩大 `scroll.x` 掩盖列消失、相邻列贴合或明显过宽的问题。

只有在 Figma / PRD 完全没有提供列集合、页面按接口 fallback 推导全部列时，才适用“不要因为列很多自行删字段”；确实过宽时使用横向滚动、合理宽度和省略。Figma / PRD 已提供闭合列集合时，不添加其中未出现的响应字段。

## 详情 Drawer 生成规则

Figma / PRD 未明确 Drawer 宽度时，所有生成的业务详情 Drawer 和关系子 Drawer 默认使用视口宽度的 `50%`：

- 优先使用已经默认 `width="50%"` 的 `BizDetailDrawer` / `BizRelationListDrawer`，调用页面不重复传 `width`。
- 禁止无设计依据写死 `640px`、`880px` 等固定宽度；完整可读 Figma 或 PRD 明确给出尺寸时才覆盖默认值。
- 子 Drawer 的 `50%` 仍相对视口计算，不相对父 Drawer 计算。

生成详情前先执行实体级复用检查，不能直接从当前页面接口开始拼 Drawer：

1. 从详情对象及稳定 ID 判断实体，例如多个页面都以 `user_id` 打开用户详情时，它们属于同一实体详情。
2. 搜索 `src/widgets/**/<Entity>Detail*`、`src/features/**/<Entity>Detail*`、切片公开入口、详情接口调用方，以及其它页面的详情操作。
3. 已有实体详情组件时直接复用。当前页面只维护目标 ID 和开关状态；从日志进入需要默认展示日志 Tab 时传 `defaultTab`，不得复制详情请求、字段、Tab 和样式。
4. 已有多个同一实体的平行详情实现时，本次点名范围覆盖这些页面的，收敛到信息更完整、调用方更多或高优先级来源指定的实现，并迁移为跨页面 widget；范围外调用方只列出，不擅自扩面。
5. 没有实体详情组件时，才使用 `@widgets/biz-detail-drawer` 的 `BizDetailDrawer` 创建；一旦同一实体出现第二个调用页面，就抽成 `<Entity>DetailDrawer` 并通过公开入口复用。

具体纯详情 / 多分组 / 多 Tab / 操作记录表格用法遵守 `docs/skills/component-usage/SKILL.md`。
...
关系列表接口不属于默认多 Tab 触发条件；由详情字段中的数量 / 箭头进入时按下节使用子 Drawer。
...
- 例如“基本信息 / 通讯录 / 群组 / 操作记录”中，通讯录和群组已由好友数量、群组数量钻取时，父详情只生成“基本信息”和“操作记录”，不保留中间两个关系 Tab。
...
- Figma / PRD 未明确宽度时，父详情与关系子 Drawer 是否均保持视口宽度 `50%`，且没有页面级固定像素宽度。
...
- 是否先精简操作文案，再逐列分析最终中英文表头、格式化内容和基础宽度比例，且日期 / 状态 / 操作没有截断或显著空白。
