...
本仓库 ESLint 排除生成目录，避免格式规则驱动人工修改只读文件。生成结果仍由生成命令、TypeScript 检查和实际调用方验证；生成物出现结构或代码风格问题时，应修正 OpenAPI 源或生成配置后重新生成。
...
4. 新增接口没有现有调用方时，先判断它是否构成可独立落页的新业务能力。能够识别出独立管理对象、列表能力或明确菜单 / 路由目标时，按 `admin-page` 和现有路由、组件、locale 模式生成页面与必要配套文件。
5. 单个详情、创建、更新、状态动作、上传、鉴权、记录或其它辅助接口不单独触发页面生成；保留生成契约，等待现有调用方接入或用户提供明确页面目标。
6. 上述完整生成和直接调用方同步属于 OpenAPI 工作流的必要范围，不视为“顺手扩修同类问题”；无调用关系的页面和无关工程债仍不得修改。
...
- 无调用方的新增接口是否只在构成可独立落页的新业务能力时生成页面。
